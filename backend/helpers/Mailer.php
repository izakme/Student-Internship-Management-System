...
<body style="font-family: Arial, sans-serif; padding: 20px; background: #f4f8fb;">
    <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
        <div style="background: linear-gradient(135deg, #0d6efd, #6610f2); padding: 20px; text-align: center;">
            <h1 style="color: white; margin: 0; font-size: 24px;">' . __("Student Internship Management System") . '</h1>
        </div>
        <div style="padding: 30px;">
            ' . $body . '
            <hr style="border: none; border-top: 1px solid #e5edf5; margin: 20px 0;">
            <p style="color: #888; font-size: 12px;">' . __("This is an automated message from SIMS. Please do not reply directly.") . '</p>
        </div>
    </div>
</body>
</html>';
    ...
    public function sendApplicationStatus($to, $studentName, $internshipTitle, $status)
    {
        ...
        $body = '<h2 style="color: #333;">' . __("Application Status Update") . '</h2>
            <p>Dear <strong>' . $studentName . '</strong>,</p>
            <p>' . __("The status of your application for") . ' <strong>' . $internshipTitle . '</strong> ' . __("has been updated.") . '</p>
            ...
            <p>' . __("Log in to your student dashboard to view more details.") . '</p>
        ...
        return $this->send($to, __("Application Status") . ': ' . $status, $body);
    }
    ...
    public function sendNewApplication($to, $companyName, $studentName, $internshipTitle)
    {
        ...
        $body = '<h2 style="color: #333;">' . __("New Application Received") . '</h2>
            <p>Dear <strong>' . $companyName . '</strong>,</p>
            <p>' . __("A new application has been submitted for your internship listing.") . '</p>
            <table style="width: 100%; border-collapse: collapse; margin: 15px 0;">
                <tr><td style="padding: 8px; background: #f4f8fb; font-weight: bold;">' . __("Student") . '</td><td style="padding: 8px;">' . $studentName . '</td></tr>
                <tr><td style="padding: 8px; background: #f4f8fb; font-weight: bold;">' . __("Internship") . '</td><td style="padding: 8px;">' . $internshipTitle . '</td></tr>
            </table>
            <p>' . __("Log in to your company dashboard to review this application.") . '</p>
            <p style="margin-top: 20px;">
                <a href="' . $loginUrl . '"
                   style="display: inline-block; padding: 12px 24px; background: #0d6efd; color: white; text-decoration: none; border-radius: 6px;">
                    ' . __("Login to Dashboard") . '
        ...
        return $this->send($to, __("New Application") . ': ' . $studentName, $body);
    }
}
